...
$(document).ready(function(){
  ...
  $('#datagridExample').datagrid({
    dataSource: {
        cols:[
			{name: 'id', label: '评论id', width: 70},
            {name: 'article', label: '所在文章', width: 200},
            {name: 'user', label: '用户名', width: 200},
            {name: 'reply', label: '评论内容', width: 500},

        ],
        array:[
			<c:forEach items="${list}" var="list">
			{ id:"${list.id}",article:"${list.articleName}",user: "${list.stuName}", reply: "${list.description}"},   		
			</c:forEach>
        ]
    },
    //特殊列的参数
...
function al(){
	...
	var xmlhttp;
	 ...
	 xmlhttp.open("POST","${ctx }/bg/deleteComment",true);
