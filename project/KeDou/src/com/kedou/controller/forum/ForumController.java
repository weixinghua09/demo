...
public class ForumController {
	...
	public String showForum(HttpSession session){
		...
		System.out.println(forumpic.get(i).getId());
		session.setAttribute("piclist",forumpic);
		
		
		List<HotSpot> hotspot = forumServiceImpl.findAllSpot();//得到热点文字列表
		session.setAttribute("hotspotlist",hotspot);
		
		
		List<Article> Article = forumServiceImpl.findAllArticle();//得到文章列表
		session.setAttribute("Articlelist",Article);
		
		List<Article> recomlist = forumServiceImpl.findRecommendArticle();//得到推荐文章列表
		session.setAttribute("relist",recomlist);
...
//        SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd");
	 ...
	 * @return
	 */
	@RequestMapping(value="/endpreview",method=RequestMethod.GET)
	...
	@ResponseBody
	public String collectArticle(String keyword,HttpSession session){
//		User u = (User)session.getAttribute("loginUser");
//		int id = u.getUserId();
		int key = Integer.parseInt(keyword);
		int line = this.forumServiceImpl.collectArticle(key, 1);
		Gson g = new Gson();
		String m = "";
		if(line != 0){
			String s = "yes";
...
//		ArrayList<HotSpot> hotspot = this.forumServiceImpl.getHotSpotByArticleId(id);
//		session.setAttribute("hotsopts", hotspot);
//		
//		try {
//			model.addAttribute("somecomments",this.forumServiceImpl.findComment(a.getId()));
//		} catch (Exception e) {
//			e.printStackTrace();
//		} 
//		
...
//		c.setComId(1);
//		try {
//			session.setAttribute("someanswers", this.forumServiceImpl.findanswer(c.getComId()));
//		} catch (Exception e) {
...
//		}
	...
	@ResponseBody
	public String saveComment(Comment comment,Model model)throws Exception{
	...
	public String deleteComment(int id ,HttpSession session)throws Exception{
		forumServiceImpl.deleteComment(id);
		return "comment";
	}
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/answertocomments", method=RequestMethod.POST)
	@ResponseBody
	public String answerToComment(String bbs)throws Exception{
		System.out.println("in");
		System.out.print(bbs);
		forumServiceImpl.answerToComment(1, bbs, "", "");
	 ...
	 * @throws Exception
	 */
	public List<Answer> findanswer(HttpSession session,int comId)throws Exception{
		Comment c = new Comment();
		c.setComId(1);
		session.setAttribute("someanswers", forumServiceImpl.findanswer(c.getComId()));
		
		return forumServiceImpl.findanswer(comId);
	}
